...
class DbHelper {
  ...
  getTest () {
    let sqlQuery = 'SELECT name FROM test WHERE id = 1'
    return this.execQuery(sqlQuery, null)
  }

  getSubmissions () {
    let sqlQuery = 'SELECT * FROM connection_request'
  ...
  insertRecommendMentorSubmission (data) {
    ...
    let values = `${data.name}, ${data.email}, ${data.mobile}, ${data.social_profiles}, ${new Date()}`
    return this.execQuery(sqlQuery, values, logger.forms)
  }
  ...
  insertRequestMetaData (data) {
    ...
    let values = `${data.requestId}, ${data.userId}, ${data.language}, ${new Date()}`
    return this.execQuery(sqlQuery, values, logger.forms)
  }
